...
export function getFestivalDateLabel(
  ...
  if (dates.length === 1) {
    return formatDisplayDate(startDate, DateTime.DATE_MED_WITH_WEEKDAY);
  }

  return `${formatDisplayDate(startDate, DateTime.DATE_MED_WITH_WEEKDAY)} a ${formatDisplayDate(endDate, DateTime.DATE_MED_WITH_WEEKDAY)}`;
}
...
export function profileHasReservationMade(
  profile: ProfileType | ProfileWithParticipationsAndRequests,
  festivalId: number,
) {
  return profile?.participations?.some((participation) => {
