...
export async function exportFestivalMapAsTemplate(
  ...
  try {
    ...
    let sectorsQuery = db.query.festivalSectors.findMany({
...
export async function importTemplateToFestival(
  ...
  try {
    ...
    if (targetSectors.length === 0) {
      return {
        success: false,
        message: "El festival no tiene sectores configurados",
      };
    }
    ...
    // Full festival import - match sectors by order
    // Check existing stands
    ...
    // Validate sector count matches
    if (template.sectors.length > targetSectors.length) {
      return {
        success: false,
        message: `La plantilla tiene ${template.sectors.length} sectores pero el festival solo tiene ${targetSectors.length}`,
      };
    }
    ...
    return await db.transaction(async (tx) => {
      ...
      // Delete all existing stands if replace mode
      if (parsedOptions.mode === "replace" && allExistingStands.length > 0) {
