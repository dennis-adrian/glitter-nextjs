...
const createStandsSchema = z.object({
  ...
  status: z.enum(["available", "reserved", "confirmed", "disabled"]).default("disabled"),
...
export async function createStands(
  ...
  try {
    ...
    return { success: false, message: "Error al crear los espacios", stands: [] };
  }
}
...
export async function deleteStands(
  ...
  try {
    ...
    // Check for any reservations on these stands
    ...
    await db.delete(stands).where(inArray(stands.id, parsed));
