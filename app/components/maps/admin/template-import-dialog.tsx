		...
		reader.onload = (e) => {
			try {
				...
				if (error instanceof SyntaxError) {
					...
					setUploadError(
						"El archivo no tiene el formato de plantilla correcto",
					);
				}
			}
		};
									...
									>
										...
										<div className="flex-1 min-w-0">
											...
											<p className="text-xs text-muted-foreground mt-1">
												...
												{template.templateData.sectors.length !== 1 &&
													"es"} •{" "}
					...
					<div className="space-y-4 pt-2 border-t">
						...
						<div className="text-sm text-muted-foreground space-y-1">
							{targetSectorId === "all" && selectedTemplate && (
								<p>
									Se configurarán{" "}
									<strong>{selectedTemplate.sectors.length}</strong> sector
									{selectedTemplate.sectors.length !== 1 && "es"} con{" "}
									<strong>{getTotalStandsInTemplate()}</strong> espacios en
									total.
									{importMode === "replace" &&
										sectors.length !== selectedTemplate.sectors.length && (
											<span>
												{" "}
												Los sectores del festival se ajustarán para coincidir
												con la plantilla.
											</span>
										)}
								</p>
