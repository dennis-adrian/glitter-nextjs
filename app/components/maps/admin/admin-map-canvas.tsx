...
function computeSnappedPosition(
  ...
  for (const [id, pos] of allPositions.entries()) {
    ...
    const xChecks = [
      ...
      { dragVal: draggedCenter.x, otherVal: otherCenter.x, guidePos: otherCenter.x }, // center-center
    ...
    const yChecks = [
      ...
      { dragVal: draggedCenter.y, otherVal: otherCenter.y, guidePos: otherCenter.y }, // center-center
...
const AdminMapCanvas = forwardRef<AdminMapCanvasHandle, AdminMapCanvasProps>(
  ...
  const [hoveredEdge, setHoveredEdge] = useState<Edge | "corner" | null>(null);
  ...
  if (showGrid && gridSize > 0) {
    const startX =
      Math.ceil(bounds.minX / gridSize) * gridSize;
    const endX = bounds.minX + bounds.width;
    const startY =
      Math.ceil(bounds.minY / gridSize) * gridSize;
  ...
  return (
    ...
    >
      ...
      {(
        [
          { edges: ["left", "top"] as Edge[], x: bounds.minX, y: bounds.minY, cursor: "nwse-resize" },
          { edges: ["right", "top"] as Edge[], x: bounds.minX + bounds.width, y: bounds.minY, cursor: "nesw-resize" },
          { edges: ["left", "bottom"] as Edge[], x: bounds.minX, y: bounds.minY + bounds.height, cursor: "nesw-resize" },
          { edges: ["right", "bottom"] as Edge[], x: bounds.minX + bounds.width, y: bounds.minY + bounds.height, cursor: "nwse-resize" },
        ] as const
        ...
        <rect
          ...
          rx={handleSize * 0.2}
          fill={hoveredEdge === "corner" ? "rgba(59, 130, 246, 0.3)" : "rgba(59, 130, 246, 0.15)"}
          stroke="rgba(59, 130, 246, 0.6)"
